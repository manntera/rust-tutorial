...
/// テスト用の破損画像ファイルを作成
fn create_corrupted_image_file(filename: &str) -> (TempDir, PathBuf) {
    let temp_dir = TempDir::new().expect("Failed to create temp directory");
    let file_path = temp_dir.path().join(filename);
    // 意図的に不正なPNGヘッダーで破損ファイルを作成
    let corrupted_data = b"INVALID_PNG_DATA";
    fs::write(&file_path, corrupted_data).expect("Failed to write corrupted file");
    (temp_dir, file_path)
}

/// 読み込み不可能なファイルを作成（権限エラー用）
fn create_unreadable_file(filename: &str) -> (TempDir, PathBuf) {
    let temp_dir = TempDir::new().expect("Failed to create temp directory");
    let file_path = temp_dir.path().join(filename);
    fs::write(&file_path, b"some content").expect("Failed to write file");
...
#[tokio::test]
async fn test_nonexistent_directory_error() {
...
async fn test_empty_directory_handling() {
    ...
    let result = engine.process_directory(empty_dir_path).await.unwrap();
...
#[tokio::test]
async fn test_corrupted_image_file_error() {
    let temp_dir = TempDir::new().unwrap();
    let (_temp_file, corrupted_file) = create_corrupted_image_file("corrupted.png");
    fs::copy(&corrupted_file, temp_dir.path().join("corrupted.png")).unwrap();
    ...
    let result = engine.process_directory(temp_dir.path().to_str().unwrap()).await.unwrap();
...
#[tokio::test]
async fn test_mixed_valid_invalid_files() {
    let temp_dir = TempDir::new().unwrap();
    ...
    // 有効なPNGファイルを作成
    fs::write(temp_dir.path().join("valid.png"), MINIMAL_PNG_DATA).unwrap();
    
    // 破損したファイルを作成
    fs::write(temp_dir.path().join("corrupted.png"), b"INVALID_PNG_DATA").unwrap();
    
    // テキストファイル（画像ではない）を作成
    fs::write(temp_dir.path().join("text.txt"), b"This is not an image").unwrap();
...
async fn test_permission_error() {
    ...
    fs::copy(&unreadable_file, temp_dir.path().join("unreadable.png")).unwrap();
...
#[tokio::test]
async fn test_invalid_configuration_error() {
    // 並列タスク数0で設定エラーをテスト
...
#[tokio::test]
async fn test_concurrent_access_error_resilience() {
    let temp_dir = TempDir::new().unwrap();
