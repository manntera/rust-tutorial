...
use crate::core::{
    HashPersistence, ProcessingConfig, ProcessingError, ProcessingResult, ProgressReporter,
};
...
impl Default for DependencyConfig {
    fn default() -> Self {
        Self {
            image_loader: ComponentConfig::new(
                "standard",
                serde_json::json!({
                    "max_dimension": 512
                }),
            ),
            perceptual_hash: ComponentConfig::new(
                "dct",
                serde_json::json!({
                    "size": 8,
                    "quality_factor": 1.0
            ...
            processing_config: ComponentConfig::new(
                "default",
                serde_json::json!({
                    "max_concurrent": null,
                    "buffer_size": 100,
            ...
            progress_reporter: ComponentConfig::new(
                "console",
                serde_json::json!({
                    "quiet": false
                }),
            ),
            hash_persistence: ComponentConfig::new(
                "streaming_json",
                serde_json::json!({
                    "buffer_size": 100
                }),
...
impl DependencyContainer {
    ...
    pub fn from_config_file(path: &std::path::Path) -> ProcessingResult<Self> {
        let config_str = std::fs::read_to_string(path).map_err(|e| {
            ProcessingError::configuration(format!("設定ファイル読み込みエラー: {e}"))
        })?;
    ...
    pub fn create_image_loader(&self) -> ProcessingResult<Box<dyn ImageLoaderBackend>> {
        let factory = crate::factories::ImageLoaderFactory::new();
        factory.create(&self.config.image_loader).map_err(|e| {
            ProcessingError::dependency_injection(format!("ImageLoader作成エラー: {e}"))
        })
    }
    ...
    pub fn create_perceptual_hash(&self) -> ProcessingResult<Box<dyn PerceptualHashBackend>> {
        let factory = crate::factories::PerceptualHashFactory::new();
        factory.create(&self.config.perceptual_hash).map_err(|e| {
            ProcessingError::dependency_injection(format!("PerceptualHash作成エラー: {e}"))
        })
    }
    ...
    pub fn create_processing_config(&self) -> ProcessingResult<Box<dyn ProcessingConfig>> {
        let factory = crate::factories::ProcessingConfigFactory::new();
        factory.create(&self.config.processing_config).map_err(|e| {
            ProcessingError::dependency_injection(format!("ProcessingConfig作成エラー: {e}"))
        })
    }
    ...
    pub fn create_progress_reporter(&self) -> ProcessingResult<Box<dyn ProgressReporter>> {
        let factory = crate::factories::ProgressReporterFactory::new();
        factory.create(&self.config.progress_reporter).map_err(|e| {
            ProcessingError::dependency_injection(format!("ProgressReporter作成エラー: {e}"))
        })
    }

    /// HashPersistenceを作成
    pub fn create_hash_persistence(
        &self,
        output_path: &std::path::Path,
    ) -> ProcessingResult<Box<dyn HashPersistence>> {
        let factory = crate::factories::HashPersistenceFactory::new();
        factory
            .create(&self.config.hash_persistence, output_path)
            .map_err(|e| {
                ProcessingError::dependency_injection(format!("HashPersistence作成エラー: {e}"))
            })
    }
    ...
    pub fn resolve_all_dependencies(
        &self,
        output_path: &std::path::Path,
    ) -> ProcessingResult<DependencyBundle> {
        Ok(DependencyBundle {
...
mod tests {
    ...
    fn test_component_config_update() {
        ...
        container
            .update_component_config("hash_persistence", new_config)
            .unwrap();
    ...
    fn test_config_serialization() {
        ...
        assert_eq!(
            config.image_loader.implementation,
            deserialized.image_loader.implementation
        );
        assert_eq!(
            config.perceptual_hash.implementation,
            deserialized.perceptual_hash.implementation
        );
    }
    ...
    fn test_config_file_creation() {
        ...
        assert_eq!(
            loaded_container.config().image_loader.implementation,
            "standard"
        );
    }
    ...
    fn test_config_presets() {
        ...
        assert_eq!(
            default_container.config().image_loader.implementation,
            "standard"
        );
        ...
        assert_eq!(
            high_perf_container.config().perceptual_hash.implementation,
            "dct"
        );
        ...
        assert_eq!(
            test_container.config().progress_reporter.implementation,
            "noop"
        );
    ...
    fn test_testing_config() {
        ...
        assert_eq!(
            config.processing_config.parameters["enable_progress"],
            false
        );
    }
    ...
    fn test_builder_presets() {
        ...
        assert_eq!(
            container.config().processing_config.parameters["max_concurrent"],
            16
        );
