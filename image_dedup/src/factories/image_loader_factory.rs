...
use crate::image_loader::{ImageLoaderBackend, standard::StandardImageLoader};
use anyhow::Result;
...
impl ComponentFactory<Box<dyn ImageLoaderBackend>> for ImageLoaderFactory {
    fn create(&self, config: &ComponentConfig) -> Result<Box<dyn ImageLoaderBackend>> {
        match config.implementation.as_str() {
            "standard" => {
                let max_dimension = config.parameters
                    .get("max_dimension")
                    .and_then(|v| v.as_u64())
                    .unwrap_or(512) as u32;
                
                Ok(Box::new(StandardImageLoader::with_max_dimension(max_dimension)))
            }
            _ => anyhow::bail!(
