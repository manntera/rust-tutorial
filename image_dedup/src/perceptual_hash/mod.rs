...
#[deprecated(note = "Use PerceptualHashFactory and PerceptualHashBackend instead")]
pub struct PerceptualHasher {
    #[allow(dead_code)]
    backend: Box<dyn PerceptualHashBackend>,
}
...
impl PerceptualHasher {
    pub fn new() -> Self {
        let backend = dct_hash::DCTHasher::new(8);
        Self {
            backend: Box::new(backend),
        }
    }

    pub fn generate_hash(&self, image: &DynamicImage) -> Result<img_hash::ImageHash> {
        // 旧APIとの互換性のため、img_hash::ImageHashを返す
    ...
    pub fn calculate_distance(hash1: &img_hash::ImageHash, hash2: &img_hash::ImageHash) -> u32 {
        hash1.dist(hash2)
    }

    pub fn are_similar(
        hash1: &img_hash::ImageHash,
        hash2: &img_hash::ImageHash,
        threshold: u32,
    ) -> bool {
