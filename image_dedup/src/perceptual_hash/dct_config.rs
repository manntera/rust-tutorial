    ...
    fn parameter_info() -> Vec<ParameterInfo> {
        vec![
            ...
            ParameterInfo {
                ...
                param_type: ParameterType::Float { min: Some(0.0), max: Some(1.0) },
...
mod tests {
    ...
    fn test_dct_config_validation() {
        // 有効な設定
        let valid_config = DctConfig { size: 16, quality_factor: 0.8 };
        assert!(valid_config.validate().is_ok());
        
        // 無効なサイズ
        let invalid_size = DctConfig { size: 0, quality_factor: 1.0 };
        assert!(invalid_size.validate().is_err());
        
        let too_large_size = DctConfig { size: 128, quality_factor: 1.0 };
        assert!(too_large_size.validate().is_err());
        
        // 無効な品質係数
        let invalid_quality = DctConfig { size: 8, quality_factor: 1.5 };
        assert!(invalid_quality.validate().is_err());
        
        let zero_quality = DctConfig { size: 8, quality_factor: 0.0 };
        assert!(zero_quality.validate().is_err());
    }
    
    #[test]
    fn test_dct_config_json_serialization() {
        let config = DctConfig { size: 16, quality_factor: 0.9 };
